...
MOD = 10007                             # 모듈러 계산을 위해 나눌 값 저장

def fill_tile(n):                       # 입력 크기만큼 타일 채우는 함수
    if n == 1:                          # 1일 때는 바로 값 리턴 (인덱스 에러 방지)
        return 1

    dp = [0] * (n + 1)                  # n의 경우 값을 넣을 공간을 크기만큼 리스트에 할당

    dp[1] = 1                           # n=1이면 2x1이므로 1가지
    dp[2] = 3                           # n=2이면 2x2이므로 3가지

    for i in range(3, n + 1):           # n=3부터 하나씩 계산
        dp[i] = dp[i-2] * 2 + dp[i-1]   # 2x전전값 + 이전 값 저장
        dp[i] %= MOD                    # 중간 과정에서 MOD연산을 통해 숫자를 줄임

    return dp[n]                        # 2xn이 되는 값 리턴
