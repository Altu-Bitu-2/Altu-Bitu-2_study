...
# Union 연산
def union(x, y):
    px = find_parent(x) # 원소 x의 루트 원소 px
    py = find_parent(y) # 원소 y의 루트 원소 py

    if px == py: # x,y가 같은 집합에 속하면
...
parent = [-1]*n     # 부모 리스트 -1로 초기화

for i in range(m): # i: 0, 1, ... , m-1 에 대해
    x, y = map(int, input().split()) # 플레이어가 선택한 두 점의 번호
    if not union(x, y): # 만약 사이클이 생성된다면
        # 사이클이 생성됨
        print(i+1) # 사이클이 만들어진 차례의 번호(i + 1) 출력
        break # 게임 종료
