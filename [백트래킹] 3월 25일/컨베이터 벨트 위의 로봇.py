...
def simulation(n, k, belt):
    ...
    while True:
        ...
        # 1. 회전
        robots.rotate(1)    #로봇 한칸 회전
        belt.rotate(1)      #벨트 한칸 회전

        robots[down_idx] = False  # 로봇 내리기

        # 2. 로봇 이동
        for idx in range(down_idx - 1, up_idx - 1, -1): #끝에서부터 확인
        ...
        # 3. 로봇 올리기
        if belt[up_idx] > 0: #올리는 위치에 있는 칸의 내구도가 0이 아니면 올리는 위치에 로봇 올리기
            robots[up_idx] = True
            belt[up_idx] -= 1
            if belt[up_idx] == 0:
                count += 1
